...
const DATA_DIR_OPTION_HELP =
  "JiGong data directory root (isolates state from ~/.jigong)";

program
  .name("jigongai")
  .description("JiGong CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program
  ...
  .option("--run", "Start JiGong immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your JiGong setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "jigong")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run JiGong")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the JiGong server API")
