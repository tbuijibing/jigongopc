...
import { ApiRequestError, JiGongApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("-d, --data-dir <path>", "JiGong data directory root (isolates state from ~/.jigong)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the JiGong API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.Jigong_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.Jigong_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.Jigong_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set Jigong_COMPANY_ID, or set context profile companyId via `jigongai context set`.",
    );
  }

  const api = new JiGongApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
