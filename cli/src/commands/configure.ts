...
function defaultConfig(): JiGongConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" jigong configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `jigongai onboard` first.");
    p.outro("");
    return;
  }

  let config: JiGongConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because Jigong_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
