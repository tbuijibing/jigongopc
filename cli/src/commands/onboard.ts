...
import type { JiGongConfig } from "../config/schema.js";
...
import { printJiGongCliBanner } from "../utils/banner.js";
...
type OnboardDefaults = Pick<JiGongConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "Jigong_PUBLIC_URL",
  "DATABASE_URL",
  "Jigong_DB_BACKUP_ENABLED",
  "Jigong_DB_BACKUP_INTERVAL_MINUTES",
  "Jigong_DB_BACKUP_RETENTION_DAYS",
  "Jigong_DB_BACKUP_DIR",
  "Jigong_DEPLOYMENT_MODE",
  "Jigong_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "Jigong_ALLOWED_HOSTNAMES",
  "Jigong_AUTH_BASE_URL_MODE",
  "Jigong_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "Jigong_STORAGE_PROVIDER",
  "Jigong_STORAGE_LOCAL_DIR",
  "Jigong_STORAGE_S3_BUCKET",
  "Jigong_STORAGE_S3_REGION",
  "Jigong_STORAGE_S3_ENDPOINT",
  "Jigong_STORAGE_S3_PREFIX",
  "Jigong_STORAGE_S3_FORCE_PATH_STYLE",
  "Jigong_SECRETS_PROVIDER",
  "Jigong_SECRETS_STRICT_MODE",
  "Jigong_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveJiGongInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.Jigong_PUBLIC_URL?.trim() ||
    process.env.Jigong_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.Jigong_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.Jigong_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.Jigong_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.Jigong_ALLOWED_HOSTNAMES
    ? process.env.Jigong_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.Jigong_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.Jigong_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.Jigong_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.Jigong_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.Jigong_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.Jigong_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.Jigong_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.Jigong_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.Jigong_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.Jigong_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.Jigong_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.Jigong_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.Jigong_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.Jigong_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.Jigong_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "Jigong_DEPLOYMENT_EXPOSURE",
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  printJiGongCliBanner();
  p.intro(pc.bgCyan(pc.black(" jigongai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveJiGongInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: JiGongConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@jigongai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `jigongai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("Jigong_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.Jigong_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("Jigong_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("Jigong_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: JiGongConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: Jigong_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("jigongai run")}`,
      `Reconfigure later: ${pc.cyan("jigongai configure")}`,
      `Diagnose setup: ${pc.cyan("jigongai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.Jigong_OPEN_ON_LISTEN = "true";
