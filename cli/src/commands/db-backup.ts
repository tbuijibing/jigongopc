...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@jigongai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://jigong:jigong@127.0.0.1:${port}/jigong`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" jigong db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveJiGongInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "jigong";
