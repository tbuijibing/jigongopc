...
const DEFAULT_AGENT_JWT_ISSUER = "jigong";
const DEFAULT_AGENT_JWT_AUDIENCE = "jigong-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveJiGongInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveJiGongInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" jigong env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: JiGongConfig | null = null;
...
function collectDeploymentEnvRows(config: JiGongConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrlSource: EnvSource =
    process.env.Jigong_PUBLIC_URL
      ? "env"
      : process.env.Jigong_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const secretsProvider =
    process.env.Jigong_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.Jigong_SECRETS_STRICT_MODE ??
  ...
  const storageProvider =
    process.env.Jigong_STORAGE_PROVIDER ??
  ...
  const storageS3Bucket =
    process.env.Jigong_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "jigong";
  const storageS3Region =
    process.env.Jigong_STORAGE_S3_REGION ??
    config?.storage?.s3?.region ??
    "us-east-1";
  const storageS3Endpoint =
    process.env.Jigong_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.Jigong_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.Jigong_STORAGE_S3_FORCE_PATH_STYLE ??
    String(config?.storage?.s3?.forcePathStyle ?? false);

  const rows: EnvVarRow[] = [
    {
      key: "Jigong_AGENT_JWT_SECRET",
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from Jigong_PUBLIC_URL when possible)",
    },
    {
      key: "Jigong_AGENT_JWT_TTL_SECONDS",
      value: process.env.Jigong_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.Jigong_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "Jigong_AGENT_JWT_ISSUER",
      value: process.env.Jigong_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.Jigong_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "Jigong_AGENT_JWT_AUDIENCE",
      value: process.env.Jigong_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.Jigong_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "Jigong_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.Jigong_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "Jigong_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.Jigong_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "Jigong_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.Jigong_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "Jigong_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.Jigong_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "Jigong_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.Jigong_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "Jigong_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.Jigong_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "Jigong_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.Jigong_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "Jigong_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.Jigong_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "Jigong_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.Jigong_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "Jigong_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.Jigong_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.Jigong_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "Jigong_CONFIG",
      value: process.env.Jigong_CONFIG ?? configPath,
      source: process.env.Jigong_CONFIG ? "env" : "default",
      required: false,
