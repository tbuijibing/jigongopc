...
describe("JiGongApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new JiGongApiClient({
    ...
    expect(headers["x-jigong-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new JiGongApiClient({ apiBase: "http://localhost:3100" });
