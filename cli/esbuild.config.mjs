/**
 * esbuild configuration for building the Jigongai CLI for npm.
 *
 * Bundles all workspace packages (@Jigongai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @Jigongai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@Jigongai/")) {
      externals.add(name);
    }
  }
