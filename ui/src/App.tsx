  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">{t("auth.instanceSetup")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("auth.bootstrapInstructions")}
        </p>
        <pre className="mt-4 overflow-x-auto rounded-md border border-border bg-muted/30 p-3 text-xs">
{`pnpm Jigongai auth bootstrap-ceo`}
        </pre>
      </div>
    </div>
  );
  ...
  if (healthQuery.error) {
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-destructive">
        {healthQuery.error instanceof Error ? healthQuery.error.message : t("app.failedLoadAppState")}
      </div>
    );
  }
...
function NoCompaniesStartPage({ autoOpen = true }: { autoOpen?: boolean }) {
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">{t("auth.createFirstCompany")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("auth.getStartedDescription")}
        </p>
        <div className="mt-4">
          <Button onClick={() => openOnboarding()}>{t("auth.newCompany")}</Button>
        </div>
      </div>
    </div>
  );
