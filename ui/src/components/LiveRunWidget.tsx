...
import type { LiveEvent } from "@Jigongai/shared";
    ...
    <div className="rounded-lg border border-cyan-500/30 bg-background/80 overflow-hidden shadow-[0_0_12px_rgba(6,182,212,0.08)]">
      {runs.length > 0 ? (
        runs.map((run) => (
          <div key={run.id} className="px-3 py-2 border-b border-border/50">
            ...
            <div className="flex items-center gap-2 text-xs">
              <span className="text-muted-foreground">{t("liveRun.run")}</span>
              <Link
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                >
                  ...
                  {cancellingRunIds.has(run.id) ? t("liveRun.stopping") : t("liveRun.stop")}
                </button>
                <Link
                  to={`/agents/${run.agentId}/runs/${run.id}`}
        ...
        <div className="flex items-center px-3 py-2 border-b border-border/50">
          <span className="text-xs font-medium text-muted-foreground">{t("liveRun.recentRunUpdates")}</span>
        </div>
      )}

      <div ref={bodyRef} className="max-h-[220px] overflow-y-auto p-2 font-mono text-[11px] space-y-1">
        {recent.length === 0 && (
          <div className="text-xs text-muted-foreground">{t("liveRun.waitingForOutput")}</div>
        )}
