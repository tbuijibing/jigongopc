...
type JiGongSprite = (typeof Jigong_SPRITES)[number];
...
function spriteSize(sprite: JiGongSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = Jigong_SPRITES[(baseCol + baseRow) % Jigong_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = Jigong_SPRITES[Math.floor(Math.random() * Jigong_SPRITES.length)]!;
