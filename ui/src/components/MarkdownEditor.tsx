...
import { buildProjectMentionHref, parseProjectMentionHref } from "@Jigongai/shared";
...
export const MarkdownEditor = React.memo(forwardRef<MarkdownEditorRef, MarkdownEditorProps>(function MarkdownEditor({
    ...
    for (const node of links) {
      ...
      if (!parsed) {
        if (link.dataset.projectMention === "true") {
          ...
          link.classList.remove("Jigong-project-mention-chip");
      ...
      link.classList.add("Jigong-project-mention-chip");
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative Jigong-mdxeditor-scope",
    ...
    >
      <MDXEditor
        ...
        onChange={(next) => {
          latestValueRef.current = next;
          // Defer onChange to avoid state updates during render
          queueMicrotask(() => {
            onChange(next);
          });
        }}
        onBlur={() => onBlur?.()}
        className={cn("Jigong-mdxeditor", !bordered && "Jigong-mdxeditor--borderless")}
        contentEditableClassName={cn(
          "Jigong-mdxeditor-content focus:outline-none [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:list-item",
          contentClassName,
        )}
