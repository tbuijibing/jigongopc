...
const ISSUE_THINKING_EFFORT_OPTIONS = {
  claude_local: [
    { value: "", labelKey: "issues.newDialog.default" },
    { value: "low", labelKey: "issues.newDialog.effortLow" },
    { value: "medium", labelKey: "issues.newDialog.effortMedium" },
    { value: "high", labelKey: "issues.newDialog.effortHigh" },
  ],
  codex_local: [
    { value: "", labelKey: "issues.newDialog.default" },
    { value: "minimal", labelKey: "issues.newDialog.effortMinimal" },
    { value: "low", labelKey: "issues.newDialog.effortLow" },
    { value: "medium", labelKey: "issues.newDialog.effortMedium" },
    { value: "high", labelKey: "issues.newDialog.effortHigh" },
  ],
  opencode_local: [
    ...
    { value: "medium", labelKey: "issues.newDialog.effortMedium" },
    { value: "high", labelKey: "issues.newDialog.effortHigh" },
    { value: "max", labelKey: "issues.newDialog.effortMax" },
  ],
} as const;
...
const statuses = [
  { value: "backlog", labelKey: "issues.status.backlog", color: issueStatusText.backlog ?? issueStatusTextDefault },
  { value: "todo", labelKey: "issues.status.todo", color: issueStatusText.todo ?? issueStatusTextDefault },
  { value: "in_progress", labelKey: "issues.status.in_progress", color: issueStatusText.in_progress ?? issueStatusTextDefault },
  { value: "in_review", labelKey: "issues.status.in_review", color: issueStatusText.in_review ?? issueStatusTextDefault },
  { value: "done", labelKey: "issues.status.done", color: issueStatusText.done ?? issueStatusTextDefault },
];

const priorities = [
  { value: "critical", labelKey: "issues.priority.critical", icon: AlertTriangle, color: priorityColor.critical ?? priorityColorDefault },
  { value: "high", labelKey: "issues.priority.high", icon: ArrowUp, color: priorityColor.high ?? priorityColorDefault },
  { value: "medium", labelKey: "issues.priority.medium", icon: Minus, color: priorityColor.medium ?? priorityColorDefault },
  { value: "low", labelKey: "issues.priority.low", icon: ArrowDown, color: priorityColor.low ?? priorityColorDefault },
];
...
export function NewIssueDialog() {
  ...
  const createIssue = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      // Invalidate queries and cleanup
  ...
  useEffect(() => {
    ...
    if (newIssueDefaults.title) {
      ...
      setIssueType((draft.issueType as IssueType) || "task");
  ...
  const assigneeOptionsTitle =
    assigneeAdapterType === "claude_local"
      ? t("issues.newDialog.claudeOptions")
      : assigneeAdapterType === "codex_local"
        ? t("issues.newDialog.codexOptions")
        : assigneeAdapterType === "opencode_local"
          ? t("issues.newDialog.openCodeOptions")
        : t("issues.newDialog.agentOptions");
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t("issues.newDialog.title")}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <textarea
            ...
            placeholder={t("issues.newDialog.titlePlaceholder")}
            rows={1}
            value={title}
            onChange={(e) => {
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("issues.newDialog.model")}</div>
                  <InlineEntitySelector
                    value={assigneeModelOverride}
                    options={modelOverrideOptions}
                    placeholder={t("issues.newDialog.defaultModel")}
                    disablePortal
                    noneLabel={t("issues.newDialog.defaultModel")}
                    searchPlaceholder={t("issues.newDialog.searchModels")}
                    emptyMessage={t("issues.newDialog.noModelsFound")}
                    onChange={setAssigneeModelOverride}
                  />
                </div>
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("issues.newDialog.thinkingEffort")}</div>
                ...
                {assigneeAdapterType === "claude_local" && (
                  <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                    <div className="text-xs text-muted-foreground">{t("issues.newDialog.enableChrome")}</div>
                    <button
                      className={cn(
                ...
                <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                  <div className="text-xs text-muted-foreground">{t("issues.newDialog.useProjectWorkspace")}</div>
                  <button
                    className={cn(
                      "relative inline-flex h-5 w-9 items-center rounded-full transition-colors",
        ...
        <div className={cn("px-4 pb-2 overflow-y-auto min-h-0 border-t border-border/60 pt-3", expanded ? "flex-1" : "")}>
          <MarkdownEditor
            ...
            placeholder={t("issues.newDialog.addDescription")}
            bordered={false}
        ...
        {effectiveCompanyId && (
          <div className="px-4 pb-2 shrink-0">
            <AgentRecommendations
              ...
              selectedAgentIds={selectedWatcherIds}
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                {(() => { const m = ISSUE_TYPE_MAP[issueType]; const Icon = m.icon; return <><Icon className="h-3 w-3" />{m.label}</>; })()}
              </button>
            </PopoverTrigger>
          ...
          <Popover open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <PopoverContent className="w-44 p-1" align="start">
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                ...
                {t("issues.newDialog.startDate")}
              </button>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                <Calendar className="h-3 w-3" />
                {t("issues.newDialog.dueDate")}
              </button>
            </PopoverContent>
          </Popover>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border shrink-0">
          ...
          >
            {t("issues.newDialog.discardDraft")}
          </Button>
          <Button
            size="sm"
          ...
          >
            {createIssue.isPending ? t("issues.newDialog.creating") : t("issues.newDialog.createIssue")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
