...
// Canonical type lives in @Jigongai/adapter-utils; re-exported here
// so existing imports from this file keep working.
export type { CreateConfigValues } from "@Jigongai/adapter-utils";
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const capabilitiesParseTimerRef = useRef<NodeJS.Timeout | null>(null);
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label="Capabilities" hint={help.capabilities}>
              <div className="space-y-1.5">
                <textarea
                  value={(() => {
                    const cap = eff("identity", "capabilities", props.agent.capabilities);
                    if (!cap) return "";
                  ...
                  onChange={(e) => {
                    ...
                    // Clear any pending parse timer
                    if (capabilitiesParseTimerRef.current) {
                      clearTimeout(capabilitiesParseTimerRef.current);
                    }
                    
                    if (trimmed.length === 0) {
                      mark("identity", "capabilities", null);
                      return;
                    }
                    
                    // Debounce JSON parsing to avoid blocking on large pastes
                    capabilitiesParseTimerRef.current = setTimeout(() => {
                      try {
                        const parsed = JSON.parse(trimmed);
                        // Always mark with parsed object if JSON is valid
                        mark("identity", "capabilities", parsed);
                      } catch (err) {
                        // Invalid JSON - don't mark as dirty yet, let user finish typing
                        // This prevents sending invalid data to the server
                      }
                    }, 300);
                  }}
                  placeholder={`{\n  "languages": ["TypeScript"],\n  "frameworks": ["React"],\n  "domains": ["frontend"],\n  "tools": ["git"],\n  "customTags": []\n}`}
                  className="w-full min-h-[120px] rounded-md border border-border px-3 py-2 bg-transparent outline-none text-sm font-mono placeholder:text-muted-foreground/40 focus:border-primary focus:ring-1 focus:ring-primary"
                />
                {(() => {
                  const cap = eff("identity", "capabilities", props.agent.capabilities);
                  if (!cap) return null;
                  const currentValue = typeof cap === "string" ? cap : JSON.stringify(cap, null, 2);
                  if (!currentValue || currentValue.trim().length === 0) return null;
                  try {
                    JSON.parse(currentValue);
                    return <p className="text-xs text-green-600 dark:text-green-400">✓ Valid JSON</p>;
                  } catch (err) {
                    return <p className="text-xs text-destructive">✗ Invalid JSON - please fix syntax errors</p>;
                  }
                })()}
              </div>
            </Field>
...
const ENABLED_ADAPTER_TYPES = new Set(["claude_local", "codex_local", "opencode_local", "openclaw_gateway", "cursor"]);
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="text-[11px] text-muted-foreground/60">
        Jigong_* variables are injected automatically at runtime.
      </p>
    </div>
  );
}
