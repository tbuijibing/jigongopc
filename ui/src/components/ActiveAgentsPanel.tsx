...
import type { Issue, LiveEvent } from "@Jigongai/shared";
    ...
    <div className={cn(
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border/50">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {isActive && (
            <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">{t("activeAgents.live")}</span>
          )}
        </div>
        <Link
      ...
      <div ref={bodyRef} className="flex-1 max-h-[140px] overflow-y-auto p-2 font-mono text-[11px] space-y-1">
        {isActive && recent.length === 0 && (
          <div className="text-xs text-muted-foreground">{t("activeAgents.waitingForOutput")}</div>
        )}
        {!isActive && recent.length === 0 && (
          <div className="text-xs text-muted-foreground">
            {run.finishedAt ? t("activeAgents.finished", { time: relativeTime(run.finishedAt) }) : t("activeAgents.started", { time: relativeTime(run.createdAt) })}
          </div>
        )}
