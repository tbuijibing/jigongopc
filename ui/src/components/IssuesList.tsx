...
import { IssueTypeIcon, ISSUE_TYPE_MAP } from "./IssueTypeIcon";
...
function applyFilters(issues: Issue[], state: IssueViewState): Issue[] {
  ...
  if (state.issueTypes.length > 0) result = result.filter((i) => state.issueTypes.includes(i.issueType ?? "task"));
  return result;
}
...
function countActiveFilters(state: IssueViewState): number {
  ...
  if (state.issueTypes.length > 0) count++;
  return count;
}
...
export function IssuesList({
  ...
  const groupedContent = useMemo(() => {
    ...
    return Object.keys(groups).map((key) => ({
      key,
      label: key === "__unassigned" ? t("issuesList.unassigned") : (agentName(key) ?? key.slice(0, 8)),
      items: groups[key]!,
    }));
    ...
    <div className="space-y-4">
              ...
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                {activeFilterCount > 0 && (
                  <X
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      updateView({ statuses: [], priorities: [], assignees: [], labels: [], issueTypes: [] });
                    }}
                  />
                )}
              </Button>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {activeFilterCount > 0 && (
                    <button
                      ...
                      onClick={() => updateView({ statuses: [], priorities: [], assignees: [], labels: [], issueTypes: [] })}
                    >
                      {t("issuesList.clear")}
                    </button>
                  )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                    ...
                    <div className="space-y-0.5">
                      {statusOrder.map((s) => (
                        <label key={s} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <StatusIcon status={s} />
                          <span className="text-sm">{t(`issues.status.${s}`, statusLabel(s))}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                          <label key={p} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            ...
                            <span className="text-sm">{t(`issues.priority.${p}`, statusLabel(p))}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                    ...
                    {agents && agents.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("issuesList.assignee")}</span>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="space-y-0.5 max-h-32 overflow-y-auto">
                        {ISSUE_TYPES.map((type) => {
                          ...
                          return (
                            <label key={type} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                              <Checkbox
                                checked={viewState.issueTypes.includes(type)}
                                onCheckedChange={() => updateView({ issueTypes: toggleInArray(viewState.issueTypes, type) })}
                              />
            ...
            <Popover>
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("issuesList.sortStatus")],
                    ["priority", t("issuesList.sortPriority")],
                    ["title", t("issuesList.sortTitle")],
                    ["created", t("issuesList.sortCreated")],
                    ["updated", t("issuesList.sortUpdated")],
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("issuesList.groupStatus")],
                    ["priority", t("issuesList.groupPriority")],
                    ["assignee", t("issuesList.groupAssignee")],
                    ["none", t("issuesList.groupNone")],
            ...
            <CollapsibleContent>
              {group.items.map((issue) => (
                ...
                >
                  ...
                  <IssueTypeIcon issueType={issue.issueType ?? "task"} className="shrink-0 text-muted-foreground" />
      ...
      {showScrollBottom && (
        <button
          ...
          aria-label={t("issuesList.scrollToBottom")}
        >
