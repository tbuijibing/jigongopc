...
import { PROJECT_COLORS } from "@Jigongai/shared";
...
const projectStatuses = [
  { value: "backlog", labelKey: "projects.newDialog.status.backlog" },
  { value: "planned", labelKey: "projects.newDialog.status.planned" },
  { value: "in_progress", labelKey: "projects.newDialog.status.in_progress" },
  { value: "completed", labelKey: "projects.newDialog.status.completed" },
  { value: "cancelled", labelKey: "projects.newDialog.status.cancelled" },
];
...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localRequired && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("projects.newDialog.localPathError"));
      return;
    }
    if (repoRequired && !isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError(t("projects.newDialog.repoUrlError"));
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t("projects.newDialog.title")}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t("projects.newDialog.namePlaceholder")}
            value={name}
        ...
        <div className="px-4 pb-2">
          <MarkdownEditor
            ...
            placeholder={t("projects.newDialog.addDescription")}
            bordered={false}
        ...
        <div className="px-4 pb-3 space-y-3 border-t border-border">
          <div className="pt-3">
            <p className="text-sm font-medium">{t("projects.newDialog.workspaceTitle")}</p>
            <p className="text-xs text-muted-foreground">{t("projects.newDialog.workspaceHint")}</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t("projects.newDialog.localFolder")}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t("projects.newDialog.localFolderHint")}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t("projects.newDialog.githubRepo")}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t("projects.newDialog.githubRepoHint")}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t("projects.newDialog.both")}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t("projects.newDialog.bothHint")}</p>
            </button>
          </div>

          {(workspaceSetup === "local" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("projects.newDialog.localFolderLabel")}</label>
              <div className="flex items-center gap-2">
                <input
                  className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                  value={workspaceLocalPath}
                  onChange={(e) => setWorkspaceLocalPath(e.target.value)}
                  placeholder={t("projects.newDialog.localFolderPlaceholder")}
                />
                <ChoosePathButton />
              </div>
            </div>
          ...
          {(workspaceSetup === "repo" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("projects.newDialog.githubRepoLabel")}</label>
              <input
                className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs outline-none"
                value={workspaceRepoUrl}
                onChange={(e) => setWorkspaceRepoUrl(e.target.value)}
                placeholder={t("projects.newDialog.githubRepoPlaceholder")}
              />
            </div>
          )}
          {workspaceError && (
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("projects.newDialog.addGoal") : t("projects.newDialog.goal")}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-56 p-1" align="start">
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  {t("projects.newDialog.allGoalsSelected")}
                </div>
              )}
            </PopoverContent>
          </Popover>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={t("projects.newDialog.targetDate")}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("projects.newDialog.failedCreate")}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t("projects.newDialog.creating") : t("projects.newDialog.createProject")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
