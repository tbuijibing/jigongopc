...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="JiGong API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "JigongApiUrl",
                  String(config.JigongApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "JigongApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://Jigong.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "Jigong"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. JiGong persists a device key during onboarding so pairing approvals
