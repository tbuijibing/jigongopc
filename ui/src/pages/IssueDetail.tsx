...
import type { ActivityEvent } from "@Jigongai/shared";
import type { Agent, IssueAttachment } from "@Jigongai/shared";
...
export function IssueDetail() {
  ...
  useEffect(() => {
    const titleLabel = issue?.title ?? issueId ?? t("nav.issues");
    setBreadcrumbs([
      { label: t("nav.issues"), href: "/issues" },
      { label: hasLiveRuns ? `🔵 ${titleLabel}` : titleLabel },
    ]);
  }, [setBreadcrumbs, issue, issueId, hasLiveRuns, t]);
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-medium text-muted-foreground">{t("issues.attachments")}</h3>
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {uploadAttachment.isPending ? t("issues.uploading") : t("issues.uploadImage")}
            </Button>
          </div>
        </div>
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        ...
        <TabsContent value="comments">
          <CommentThread
            ...
            agentMap={agentMap}
            draftKey={`Jigong:issue-comment-draft:${issue.id}`}
            enableReassign
      ...
      <DependencyPanel issueId={issue.id} />

      <WatcherPanel issueId={issue.id} />
        ...
        >
          <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
            <span className="text-sm font-medium text-muted-foreground">
              {t("issues.linkedApprovals", { count: linkedApprovals.length })}
            </span>
            <ChevronDown
      ...
      {linkedRuns && linkedRuns.length > 0 && (
        ...
        >
          <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
            <span className="text-sm font-medium text-muted-foreground">{t("issues.costSummary")}</span>
            <ChevronDown
              ...
              {!issueCostSummary.hasCost && !issueCostSummary.hasTokens ? (
                ...
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                  ...
                  {issueCostSummary.hasTokens && (
                    <span>
                      {t("issues.tokens")} {formatTokens(issueCostSummary.totalTokens)}
