...
const PRESET_LABELS: Record<DatePreset, string> = {
  mtd: "costs.monthToDate",
  "7d": "costs.last7Days",
  "30d": "costs.last30Days",
  ytd: "costs.yearToDate",
  all: "costs.allTime",
  custom: "costs.custom",
};
      ...
      <div className="flex flex-wrap items-center gap-2">
        {presetKeys.map((p) => (
          <Button
            ...
            onClick={() => setPreset(p)}
          >
            {PRESET_LABELS[p] ? t(PRESET_LABELS[p]) : p}
          </Button>
        ))}
        {preset === "custom" && (
              ...
              <div className="flex items-center justify-between">
                ...
                {data.summary.budgetCents > 0 && (
                  <p className="text-sm text-muted-foreground">
                    {t("costs.utilized", { percent: data.summary.utilizationPercent })}
                  </p>
                )}
              </div>
                          ...
                          {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) && (
                            <span className="text-xs text-muted-foreground block">
                              ...
                              {row.subscriptionRunCount > 0
                                ? t("costs.subscriptionRuns", { count: row.subscriptionRunCount, inputTokens: formatTokens(row.subscriptionInputTokens), outputTokens: formatTokens(row.subscriptionOutputTokens) })
                                : null}
                            </span>
                          )}
                        </div>
