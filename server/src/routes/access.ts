...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (normalized !== "jigong" && normalized !== "jigong-create-agent")
    return null;
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.jigongApiUrl)) {
    const legacyJiGongApiUrl = nonEmptyTrimmedString(input.jigongApiUrl);
    if (legacyJiGongApiUrl) merged.jigongApiUrl = legacyJiGongApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    jigongApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.jigongApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawJiGongApiUrl =
    typeof defaults.jigongApiUrl === "string"
      ? defaults.jigongApiUrl.trim()
      : "";
  if (rawJiGongApiUrl) {
    try {
      const parsedJiGongApiUrl = new URL(rawJiGongApiUrl);
      if (
        parsedJiGongApiUrl.protocol !== "http:" &&
        parsedJiGongApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_jigong_api_url_protocol",
          level: "warn",
          message: `jigongApiUrl must use http:// or https:// (got ${parsedJiGongApiUrl.protocol}).`
        });
      } else {
        normalized.jigongApiUrl = parsedJiGongApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_jigong_api_url_configured",
          level: "info",
          message: `jigongApiUrl set to ${parsedJiGongApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_jigong_api_url_invalid",
        level: "warn",
        message: `Invalid jigongApiUrl: ${rawJiGongApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your JiGong host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "JiGong is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm jigongai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/jigong";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/jigong-claimed-api-key.json and load Jigong_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: jigongApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the JiGong hostname is reachable and allowed via `pnpm jigongai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this JiGong API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/jigong/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # JiGong OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, JiGong generates and persists one during join so pairing approvals are stable.
    ...
    The board approves the join request in JiGong before key claim is allowed.
    ...
    - ~/.openclaw/workspace/jigong-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/jigong-claimed-api-key.json

    And set the Jigong_API_KEY and Jigong_API_URL in your environment variables as specified here:
    ...
    Then set Jigong_API_KEY and Jigong_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install JiGong skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your Jigong_API_URL to the top of your skill and note the path to your Jigong_API_URL
    ...
    ${
      ...
      "Ensure JiGong is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested JiGong base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.jigongApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm jigongai allowed-hostname <host>
      - then restart JiGong and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@jigong.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "jigong", path: "/api/skills/jigong" },
        {
          name: "jigong-create-agent",
          path: "/api/skills/jigong-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              jigongApiUrl: req.body.jigongApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.jigongApiUrl &&
          !persistedDefaults.jigongApiUrl
        ) {
          missingPersistedFields.push("jigongApiUrl");
        }
  ...
  router.get("/companies/:companyId/members/list", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    const members = await access.listMembersWithUsers(companyId);
    res.json(members);
  });
