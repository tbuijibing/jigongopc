...
import { agents as agentsTable, companies, heartbeatRuns } from "@jigongai/db";
...
import { runClaudeLogin } from "@jigongai/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@jigongai/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@jigongai/adapter-cursor-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@jigongai/adapter-opencode-local/server";
...
export function agentRoutes(db: Db) {
  ...
  const heartbeatConfigSvc = heartbeatConfigService(db);
  const soulSvc = soulService(db);
  ...
  // ---------------------------------------------------------------------------
  // Capability Discovery routes (Requirements 6.2, 6.3, 6.5)
  // IMPORTANT: These must be registered BEFORE :agentId routes so Express
  // does not treat "capabilities" / "discover" as an agentId value.
  // ---------------------------------------------------------------------------

  router.get("/companies/:companyId/agents/capabilities", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
  ...
  router.get("/companies/:companyId/agents/discover", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);

    const needParam = typeof req.query.need === "string" ? req.query.need.trim() : "";
    const needTokens = needParam.length > 0
      ? needParam.split(",").map((s) => s.trim()).filter((s) => s.length > 0)
      : [];

    if (needTokens.length === 0) {
      // No filter — return all non-terminated agents via discoverAgents with empty needs
      const agents = await svc.discoverAgents(companyId, {});
      res.json(agents);
      return;
    }

    // For the generic ?need= query, each token must match in at least one
    // capability dimension. Fetch the full matrix and filter here so we can
    // apply the "every need matches some dimension" semantics.
    const matrix = await svc.getCapabilitiesMatrix(companyId);
  ...
  // ---------------------------------------------------------------------------
  // Heartbeat Config routes (Requirements 1.3, 1.6)
  // ---------------------------------------------------------------------------

  router.get("/companies/:companyId/agents/:agentId/heartbeat-config", async (req, res) => {
    const { companyId, agentId } = req.params;
    assertCompanyAccess(req, companyId);
    const result = await heartbeatConfigSvc.getConfig(companyId, agentId);
    res.json(result);
  });

  router.put("/companies/:companyId/agents/:agentId/heartbeat-config", async (req, res) => {
    const { companyId, agentId } = req.params;
    assertCompanyAccess(req, companyId);
    ...
    const row = await heartbeatConfigSvc.upsertConfig(companyId, agentId, req.body);
    ...
    await logActivity(db, {
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "agent.heartbeat_config_updated",
      entityType: "agent_heartbeat_config",
      entityId: row.id,
      details: { agentId },
    });
  ...
  // ---------------------------------------------------------------------------
  // Soul routes (Requirements 2.3, 2.5)
  // ---------------------------------------------------------------------------

  router.get("/companies/:companyId/agents/:agentId/soul", async (req, res) => {
    const { companyId, agentId } = req.params;
    assertCompanyAccess(req, companyId);
    const result = await soulSvc.getSoul(companyId, agentId);
    res.json(result);
  });

  router.put("/companies/:companyId/agents/:agentId/soul", async (req, res) => {
    const { companyId, agentId } = req.params;
    assertCompanyAccess(req, companyId);
    const agent = await svc.getById(agentId);
    if (!agent || agent.companyId !== companyId) {
    ...
    const row = await soulSvc.upsertSoul(companyId, agentId, {
    ...
    await logActivity(db, {
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "agent.soul_updated",
      entityType: "agent_soul",
      entityId: row.id,
      details: { agentId, version: row.version },
    });
  ...
  // ---------------------------------------------------------------------------
  // Tools CRUD routes (Requirements 3.2, 3.3, 3.6, 21.1)
  // ---------------------------------------------------------------------------

  router.get("/companies/:companyId/agents/:agentId/tools", async (req, res) => {
    const { companyId, agentId } = req.params;
    assertCompanyAccess(req, companyId);
    const tools = await agentToolSvc.listTools(companyId, agentId);
    res.json(tools);
  });

  router.post("/companies/:companyId/agents/:agentId/tools", async (req, res) => {
    const { companyId, agentId } = req.params;
    assertCompanyAccess(req, companyId);
    const agent = await svc.getById(agentId);
    if (!agent || agent.companyId !== companyId) {
    ...
    if (!body.toolType || typeof body.toolType !== "string") {
      res.status(400).json({ error: "toolType is required and must be a string" });
      return;
    }
    if (!(TOOL_TYPES as readonly string[]).includes(body.toolType)) {
      res.status(422).json({
        error: `Invalid toolType '${body.toolType}'. Must be one of: ${TOOL_TYPES.join(", ")}`,
      });
      return;
    }

    const tool = await agentToolSvc.createTool(companyId, agentId, {
    ...
    await logActivity(db, {
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "agent.tool_created",
      entityType: "agent_tool",
      entityId: tool.id,
      details: { agentId, toolType: body.toolType, name: body.name },
    });

    res.status(201).json(tool);
  });

  router.put("/companies/:companyId/agents/:agentId/tools/:toolId", async (req, res) => {
    const { companyId, agentId, toolId } = req.params;
    assertCompanyAccess(req, companyId);
    const agent = await svc.getById(agentId);
    if (!agent || agent.companyId !== companyId) {
      res.status(404).json({ error: "Agent not found" });
    ...
    const body = req.body;
    if (body.toolType !== undefined && !(TOOL_TYPES as readonly string[]).includes(body.toolType)) {
      res.status(422).json({
        error: `Invalid toolType '${body.toolType}'. Must be one of: ${TOOL_TYPES.join(", ")}`,
      });
      return;
    ...
    const tool = await agentToolSvc.updateTool(companyId, agentId, toolId, {
      name: body.name,
  ...
  router.delete("/companies/:companyId/agents/:agentId/tools/:toolId", async (req, res) => {
    const { companyId, agentId, toolId } = req.params;
    assertCompanyAccess(req, companyId);
    const agent = await svc.getById(agentId);
    if (!agent || agent.companyId !== companyId) {
    ...
    const deleted = await agentToolSvc.deleteTool(companyId, agentId, toolId);
    ...
    await logActivity(db, {
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "agent.tool_deleted",
      entityType: "agent_tool",
      entityId: toolId,
      details: { agentId, toolType: deleted.toolType, name: deleted.name },
    });
  ...
  // ---------------------------------------------------------------------------
  // Skills Store routes — company-level skill registry (Requirements 4.4, 4.9, 21.1)
  // ---------------------------------------------------------------------------

  router.get("/companies/:companyId/skills", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    const skills = await skillRegistrySvc.listSkills(companyId);
    res.json(skills);
  });
  ...
  router.post("/companies/:companyId/skills", async (req, res) => {
    ...
    if (!(SKILL_CATEGORIES as readonly string[]).includes(body.category)) {
      res.status(422).json({
        error: `Invalid category '${body.category}'. Must be one of: ${SKILL_CATEGORIES.join(", ")}`,
      });
      return;
    }

    const skill = await skillRegistrySvc.createSkill(companyId, {
      name: body.name,
      slug: body.slug,
      description: body.description ?? null,
    ...
    await logActivity(db, {
      ...
      entityId: skill.id,
      details: { name: body.name, slug: body.slug, category: body.category },
    });
  ...
  // ---------------------------------------------------------------------------
  // Agent Skills routes — install/uninstall/list (Requirements 4.4, 4.8, 4.9, 21.1)
  // ---------------------------------------------------------------------------

  router.get("/companies/:companyId/agents/:agentId/skills", async (req, res) => {
    const { companyId, agentId } = req.params;
    assertCompanyAccess(req, companyId);
    const result = await agentSkillSvc.getAgentSkills(companyId, agentId);
    res.json(result);
  });

  router.post("/companies/:companyId/agents/:agentId/skills/install", async (req, res) => {
    const { companyId, agentId } = req.params;
    assertCompanyAccess(req, companyId);
    const agent = await svc.getById(agentId);
    if (!agent || agent.companyId !== companyId) {
    ...
    if (!body.skillId || typeof body.skillId !== "string") {
      res.status(400).json({ error: "skillId is required and must be a string" });
      return;
    }

    const actor = getActorInfo(req);
    const installed = await agentSkillSvc.installSkill(
      companyId,
      agentId,
      body.skillId,
      "manual",
    ...
    await logActivity(db, {
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "agent.skill_installed",
      entityType: "agent_skill",
      entityId: installed.id,
      details: { agentId, skillId: body.skillId, installType: "manual" },
    });

    res.status(201).json(installed);
  });

  router.delete("/companies/:companyId/agents/:agentId/skills/:skillId", async (req, res) => {
    const { companyId, agentId, skillId } = req.params;
    assertCompanyAccess(req, companyId);
    const agent = await svc.getById(agentId);
    if (!agent || agent.companyId !== companyId) {
      res.status(404).json({ error: "Agent not found" });
    ...
    const deleted = await agentSkillSvc.uninstallSkill(companyId, agentId, skillId);
    ...
    await logActivity(db, {
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "agent.skill_uninstalled",
      entityType: "agent_skill",
      entityId: deleted.id,
      details: { agentId, skillId },
    });
  ...
  // ---------------------------------------------------------------------------
  // Memory CRUD routes (Requirements 5.4, 5.7, 21.1)
  // ---------------------------------------------------------------------------
  ...
  router.get("/companies/:companyId/agents/:agentId/memories", async (req, res) => {
    ...
    if (layer && !(MEMORY_LAYERS as readonly string[]).includes(layer)) {
      res.status(422).json({
        error: `Invalid layer '${layer}'. Must be one of: ${MEMORY_LAYERS.join(", ")}`,
      });
      return;
    }

    const memories = await agentMemorySvc.readMemories(companyId, agentId, {
      layer,
      scopeId,
    });
    res.json(memories);
  ...
  router.post("/companies/:companyId/agents/:agentId/memories", async (req, res) => {
    ...
    if (!body.memoryLayer || typeof body.memoryLayer !== "string") {
      res.status(400).json({ error: "memoryLayer is required and must be a string" });
      return;
    }
    if (!body.memoryType || typeof body.memoryType !== "string") {
      res.status(400).json({ error: "memoryType is required and must be a string" });
      return;
    }

    const memory = await agentMemorySvc.writeMemory(companyId, agentId, {
      key: body.key,
      value: body.value,
    ...
    await logActivity(db, {
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "agent.memory_created",
      entityType: "agent_memory",
      entityId: memory.id,
      details: { agentId, memoryLayer: body.memoryLayer, key: body.key },
    });

    res.status(201).json(memory);
  });

  router.put("/companies/:companyId/agents/:agentId/memories/:memoryId", async (req, res) => {
    const { companyId, agentId, memoryId } = req.params;
    assertCompanyAccess(req, companyId);
    const agent = await svc.getById(agentId);
    if (!agent || agent.companyId !== companyId) {
      res.status(404).json({ error: "Agent not found" });
    ...
    const body = req.body;
    const memory = await agentMemorySvc.updateMemory(companyId, agentId, memoryId, {
      value: body.value,
      memoryType: body.memoryType,
      importance: body.importance,
  ...
  router.delete("/companies/:companyId/agents/:agentId/memories/:memoryId", async (req, res) => {
    ...
    const deleted = await agentMemorySvc.deleteMemory(companyId, agentId, memoryId);
    ...
    await logActivity(db, {
      ...
      entityType: "agent_memory",
      entityId: memoryId,
      details: { agentId, memoryLayer: deleted.memoryLayer, key: deleted.key },
    });
