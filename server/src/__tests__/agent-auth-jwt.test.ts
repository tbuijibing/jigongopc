...
describe("agent local JWT", () => {
  const secretEnv = "Jigong_AGENT_JWT_SECRET";
  const ttlEnv = "Jigong_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "Jigong_AGENT_JWT_ISSUER";
  const audienceEnv = "Jigong_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "jigong";
    process.env[audienceEnv] = "jigong-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
