...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@jigongai/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@jigongai/adapter-codex-local/ui";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use jigong skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use jigong skill**" },
    ]);
  });
