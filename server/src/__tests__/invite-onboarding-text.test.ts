...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("JiGong OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested JiGong base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("jigongApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.jigongApiUrl");
    expect(text).toContain("~/.openclaw/workspace/jigong-claimed-api-key.json");
    expect(text).toContain("Jigong_API_KEY");
