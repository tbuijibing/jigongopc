...
import { execute, testEnvironment } from "@jigongai/adapter-openclaw-gateway/server";
import { parseOpenClawGatewayStdoutLine } from "@jigongai/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@jigongai/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("jigong:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("Jigong_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("Jigong_TASK_ID=task-123");
