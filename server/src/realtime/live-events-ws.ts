...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@jigongai/db";
import type { DeploymentMode } from "@jigongai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  jigongUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).jigongUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.jigongUpgradeContext = context;
