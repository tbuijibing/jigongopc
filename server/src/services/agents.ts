...
import { isUuidLike, normalizeAgentUrlKey } from "@jigongai/shared";
...
function configPatchFromSnapshot(snapshot: unknown): Partial<typeof agents.$inferInsert> {
  ...
  return {
    ...
    capabilities:
      isPlainRecord(snapshot.capabilities) || snapshot.capabilities === null
        ? (snapshot.capabilities as typeof agents.$inferInsert["capabilities"])
        : null,
...
export function agentService(db: Db) {
  ...
  return {
    ...
    getCapabilitiesMatrix: async (companyId: string) => {
      const rows = await db
        .select({
          id: agents.id,
          name: agents.name,
