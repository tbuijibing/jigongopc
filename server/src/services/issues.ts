...
export function issueService(db: Db) {
  ...
  /**
   * When a human user assigns an issue to an agent, verify the user has
   * canAssignTasks permission in human_agent_controls for that agent.
   * Validates: Requirements 16.1, 16.3
   */
  async function assertHumanCanAssignToAgent(companyId: string, userId: string, agentId: string): Promise<void> {
    const control = await db
      .select({ permissions: humanAgentControls.permissions })
      .from(humanAgentControls)
      .where(
        and(
          eq(humanAgentControls.companyId, companyId),
          eq(humanAgentControls.userId, userId),
    ...
    if (!control) {
      throw forbidden("No control relationship with this agent");
    }

    const perms = control.permissions as HumanAgentPermissions | null;
    if (!perms || !perms.canAssignTasks) {
      throw forbidden("Missing canAssignTasks permission for this agent");
    }
  }

  /**
   * Create a WakeupRequest after successfully assigning an issue to an agent.
   * Validates: Requirement 16.2
   */
  async function createAssignmentWakeup(
    companyId: string,
    agentId: string,
    issueId: string,
    actorUserId: string,
  ...
  return {
    ...
    create: async (
      ...
      if (issueData.issueType && !(ISSUE_TYPES as readonly string[]).includes(issueData.issueType)) {
        throw unprocessable(`Invalid issue type: ${issueData.issueType}. Must be one of: ${ISSUE_TYPES.join(", ")}`);
      }
    ...
    update: async (id: string, data: Partial<typeof issues.$inferInsert> & { labelIds?: string[] }, options?: { actorUserId?: string }) => {
      ...
      if (issueData.assigneeAgentId) {
        ...
        // Check human_agent_controls canAssignTasks permission (Req 16.1, 16.3)
        if (options?.actorUserId) {
          await assertHumanCanAssignToAgent(existing.companyId, options.actorUserId, issueData.assigneeAgentId);
        }
      }
      ...
      // Fire collaboration/watcher events on status change (fire-and-forget)
      if (result && issueData.status && issueData.status !== existing.status) {
        const newStatus = issueData.status;
        const collab = collaborationService(db);
        const watchers = issueWatcherService(db);

        if (newStatus === "done") {
          collab.checkParentChildCompletion(id).catch((err) => {
            console.error(`[issue-event] checkParentChildCompletion failed for issue ${id}:`, err);
          });
          collab.checkDependencyWakeup(id).catch((err) => {
            console.error(`[issue-event] checkDependencyWakeup failed for issue ${id}:`, err);
          });
        }

        watchers.notifyWatchers(id, newStatus).catch((err) => {
          console.error(`[issue-event] notifyWatchers failed for issue ${id}:`, err);
        });
      }

      // Create WakeupRequest when a human assigns an issue to an agent (Req 16.2)
      if (
        result &&
        ...
        createAssignmentWakeup(
          existing.companyId,
          issueData.assigneeAgentId,
          id,
          options.actorUserId,
        ).catch((err) => {
          console.error(`[issue-event] createAssignmentWakeup failed for issue ${id}:`, err);
        });
      }
