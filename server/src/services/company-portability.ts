...
import { normalizeAgentUrlKey, portabilityManifestSchema } from "@jigongai/shared";
import { notFound, unprocessable } from "../errors.js";

type AgentCapabilitiesJsonb = {
  languages: string[];
  frameworks: string[];
  domains: string[];
  tools: string[];
...
function parseCapabilitiesJsonb(value: string | null): AgentCapabilitiesJsonb {
  if (!value) return null;
  try {
    return JSON.parse(value) as AgentCapabilitiesJsonb;
  } catch {
    return null;
  }
}
...
async function readAgentInstructions(agent: AgentLike): Promise<{ body: string; warning: string | null }> {
  ...
  if (instructionsFilePath) {
    const workspaceCwd = asString(process.env.Jigong_WORKSPACE_CWD);
...
export function companyPortabilityService(db: Db) {
  ...
  async function resolveSource(source: CompanyPortabilityPreview["source"]): Promise<ResolvedSource> {
    ...
    let ref = parsed.ref;
    const manifestRelativePath = [parsed.basePath, "jigong.manifest.json"].filter(Boolean).join("/");
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const patch = {
          ...
          capabilities: parseCapabilitiesJsonb(manifestAgent.capabilities),
          reportsTo: null,
