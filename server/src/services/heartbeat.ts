...
const DEFERRED_WAKE_CONTEXT_KEY = "_jigongWakeContext";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.jigongWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    try {
      ...
      for (const warning of runtimeWorkspaceWarnings) {
        await onLog("stderr", `[jigong] ${warning}\n`);
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected Jigong_API_KEY",
        );
      }
      // --- Prompt Assembly: inject Agent six-dimension context ---
      try {
        const projectId = resolvedWorkspace.projectId ?? undefined;
        const injectionResults = await promptAssemblyService(db).assembleContext(
          agent.companyId,
          agent.id,
          agent.adapterType,
          { issueId: issueId ?? undefined, projectId },
        );
        if (injectionResults.length > 0) {
          context._injectionResults = injectionResults;
        }
      } catch (assemblyErr) {
        logger.warn(
          { companyId: agent.companyId, agentId: agent.id, runId: run.id, err: assemblyErr },
          "Prompt assembly failed, continuing without injection",
        );
      }
      // --- End Prompt Assembly ---
