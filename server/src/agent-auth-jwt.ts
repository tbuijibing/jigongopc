...
function jwtConfig() {
  const secret = process.env.Jigong_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.Jigong_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.Jigong_AGENT_JWT_ISSUER ?? "jigong",
    audience: process.env.Jigong_AGENT_JWT_AUDIENCE ?? "jigong-api",
  };
}
