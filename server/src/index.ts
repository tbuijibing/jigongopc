...
if (process.env.Jigong_SECRETS_PROVIDER === undefined) {
  process.env.Jigong_SECRETS_PROVIDER = config.secretsProvider;
}
if (process.env.Jigong_SECRETS_STRICT_MODE === undefined) {
  process.env.Jigong_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
}
if (process.env.Jigong_SECRETS_MASTER_KEY_FILE === undefined) {
  process.env.Jigong_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
}
...
async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
  if (process.env.Jigong_MIGRATION_PROMPT === "never") return false;
  if (process.env.Jigong_MIGRATION_AUTO_APPLY === "true") return true;
...
const LOCAL_BOARD_USER_EMAIL = "local@jigong.local";
...
if (config.databaseUrl) {
  ...
  const verboseEmbeddedPostgresLogs = process.env.Jigong_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  const embeddedAdminConnectionString = `postgres://jigong:jigong@127.0.0.1:${port}/postgres`;
  const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "jigong");
  if (dbStatus === "created") {
    logger.info("Created embedded PostgreSQL database: jigong");
  }

  const embeddedConnectionString = `postgres://jigong:jigong@127.0.0.1:${port}/jigong`;
...
if (config.deploymentMode === "authenticated") {
  ...
  const betterAuthSecret =
    process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.Jigong_AGENT_JWT_SECRET?.trim();
  if (!betterAuthSecret) {
    throw new Error(
      "authenticated mode requires BETTER_AUTH_SECRET (or Jigong_AGENT_JWT_SECRET) to be set",
    );
  }
...
const runtimeApiHost =
  runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
    ? "localhost"
    : runtimeListenHost;
process.env.Jigong_LISTEN_HOST = runtimeListenHost;
process.env.Jigong_LISTEN_PORT = String(listenPort);
process.env.Jigong_API_URL = `http://${runtimeApiHost}:${listenPort}`;
...
server.listen(listenPort, config.host, () => {
  ...
  if (process.env.Jigong_OPEN_ON_LISTEN === "true") {
