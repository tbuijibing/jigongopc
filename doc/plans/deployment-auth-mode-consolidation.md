...
Keep JiGong low-friction while making the mode model simpler and safer:
...
```sh
pnpm Jigongai configure --section server
```
...
1. `pnpm Jigongai onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm Jigongai doctor` works flagless with mode-aware checks.
