...
This document defines the V1 design for a JiGong adapter that integrates with
...
4. synthesized stdout events for JiGong UI/CLI
...
3. **Agent callback auth to JiGong** uses a bootstrap exchange flow (no long-lived JiGong key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from JiGong endpoints, not full SKILL.md prompt injection.
...
- `JigongPublicUrl` (optional override; else `Jigong_PUBLIC_URL` env)
...
## JiGong Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `Jigong_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.JigongPublicUrl`
2. `process.env.Jigong_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived JiGong credentials in prompt text.

Flow:

1. Before launch/follow-up, JiGong mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. JiGong validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all JiGong API calls.
...
1. Prompt includes a compact instruction to fetch skills from JiGong.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/Jigong`
   - `GET /api/skills/Jigong-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <JigongPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active JiGong run context.
...
5. `JigongPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from JiGong skill endpoints |
| JiGong auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from JiGong context.
3. Add richer PR metadata surfacing in JiGong UI.
