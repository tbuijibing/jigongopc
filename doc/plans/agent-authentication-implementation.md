...
- New config/env settings:
  - `Jigong_AGENT_JWT_SECRET`
  - `Jigong_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `Jigong_AGENT_JWT_ISSUER` (default: `Jigong`)
  - `Jigong_AGENT_JWT_AUDIENCE` (default: `Jigong-api`)
...
1. In:
   ...
   inject `Jigong_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `Jigong_API_KEY`, do not overwrite it.
- Continue injecting:
  - `Jigong_AGENT_ID`
  - `Jigong_COMPANY_ID`
  - `Jigong_API_URL`
