...
Agents need API keys to authenticate with JiGong. The current approach
...
**Trust model:** The adapter process runs on the same machine as the JiGong
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** JiGong generates a token and passes it directly to the agent
...
`Jigong_API_URL`, `Jigong_AGENT_ID`, `Jigong_COMPANY_ID`. We need to
add a `Jigong_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a JiGong CLI
...
```
Jigong auth login
# Opens browser -> user confirms -> token stored at ~/.Jigong/credentials
```
...
managed by the JiGong server itself.
...
1. A company admin (user or agent) generates an **invite URL** from JiGong.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The JiGong SKILL.md (or a link to it)
   - What information JiGong needs from the agent (e.g. webhook URL, adapter
...
5. JiGong stores the pending registration.
...
7. On approval, JiGong provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** JiGong issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.Jigong.ing/skills/Jigong/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How JiGong should send you heartbeats",
...
1. Generate a JiGong invite link for the company.
...
4. A JiGong company member approves the new agent.
5. JiGong begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `Jigong_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `Jigongai auth login` for developer-managed remote agents.                                      |
