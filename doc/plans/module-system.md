# JiGong Module System

## Overview

JiGong's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends JiGong's API, UI, and data model | Yes |
...
modules/
  observability/
    Jigong.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@Jigongai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@Jigongai/core";
...
```
1. Scan modules/ directory for Jigong.module.json manifests
...
```jsonc
// Jigong.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@Jigongai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@Jigongai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:Jigong/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.Jigong.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm Jigongai store list                    # browse available modules and templates
pnpm Jigongai store install <module-id>     # install a module
pnpm Jigongai store import <template-id>    # import a company template
pnpm Jigongai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one JiGong instance | `server:started` |
...
4. **Module config** — `Jigong.config.json` with per-module config, env var interpolation
...
10. **`@Jigongai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
