...
Add a single storage subsystem for JiGong that supports:
...
- API serves attachment bytes through authenticated JiGong endpoints.
...
- Default local path is under instance root: `~/.Jigong/instances/<instanceId>/data/storage`.
...
- `Jigongai onboard` writes a valid `storage` config block by default.
- `Jigongai configure --section storage` can switch between local and s3 modes.
...
- `Jigongai doctor` reports actionable storage status.
