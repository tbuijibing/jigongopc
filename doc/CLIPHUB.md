...
ClipHub is the public registry where people share, discover, and download JiGong company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to JiGong what a package registry is to a programming language. JiGong already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in JiGong — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own JiGong instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the JiGong spec. It contains:
...
**Install (fresh start):**
```
Jigong install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local JiGong instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within JiGong, export your company as a template, then publish:

```
Jigong export --template my-company
Jigong publish cliphub my-company
```
...
ClipHub is a **separate service** from JiGong itself. JiGong is self-hosted; ClipHub is a hosted registry that JiGong instances talk to.
...
| **JiGong CLI** | `Jigongai install`, `Jigongai publish`, `Jigongai cliphub sync` — built into JiGong |
| **JiGong UI** | "Browse ClipHub" panel in the JiGong web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with JiGong) |
| Backend | TypeScript + Hono (consistent with JiGong) |
| Database | PostgreSQL |
...
4. Run `Jigongai install cliphub:acme/lean-saas-shop`
5. JiGong creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in JiGong until it works well
2. Export: `Jigongai export --template my-agency`
3. Publish: `Jigongai publish cliphub my-agency`
...
3. Install just that agent: `Jigongai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to JiGong

ClipHub is **not required** to use JiGong. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to JiGong what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `Jigongai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in JiGong web UI
- [ ] `Jigongai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with JiGong's own philosophy)
