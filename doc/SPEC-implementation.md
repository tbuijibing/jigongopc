...
JiGong V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.Jigong/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.Jigong/instances/default/data/storage` (`local_disk`)
...
- exactly one JSON entrypoint: `Jigong.manifest.json`
