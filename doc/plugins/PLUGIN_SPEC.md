# JiGong Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for JiGong's plugin and extension architecture.
...
JiGong plugin design is based on the following assumptions:

1. JiGong is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core JiGong business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by JiGong core.
...
2. Let plugins add major capabilities without editing JiGong core.
...
4. Depend on project-local plugin folders such as `.Jigong/plugins`.
...
The single JiGong deployment an operator installs and controls.

### 5.2 Company

A first-class JiGong business object inside the instance.
...
A trusted in-process extension loaded directly by JiGong core.
...
An installable instance-wide extension package loaded through the JiGong plugin runtime.
...
JiGong has two extension classes.
...
JiGong already has a concrete workspace model:
...
Plugins live under the JiGong instance directory.

Suggested layout:

- `~/.Jigong/instances/default/plugins/package.json`
- `~/.Jigong/instances/default/plugins/node_modules/`
- `~/.Jigong/instances/default/plugins/.cache/`
- `~/.Jigong/instances/default/data/plugins/<plugin-id>/`

The package install directory and the plugin data directory are separate.

## 8.2 Operator Commands

JiGong should add CLI commands:

- `pnpm Jigongai plugin list`
- `pnpm Jigongai plugin install <package[@version]>`
- `pnpm Jigongai plugin uninstall <plugin-id>`
- `pnpm Jigongai plugin upgrade <plugin-id> [version]`
- `pnpm Jigongai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@Jigong/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface JiGongPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that JiGong agents can use during runs.
...
- JiGong server starts one worker process per installed plugin
...
Receives one typed JiGong domain event.
...
export function definePlugin(definition: PluginDefinition): JiGongPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: JiGongPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@Jigongai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@Jigongai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@Jigongai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@Jigongai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@Jigongai/plugin-sdk/ui`
...
The `@Jigongai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the JiGong secret provider system rather than a plain text input
...
1. Core JiGong data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing JiGong objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual JiGong product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if JiGong later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing JiGong secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm Jigongai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the JiGong server. This is a normative requirement, not optional.
...
### 27.1 `@Jigongai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@Jigongai/plugin-test-harness";
...
For developing a plugin against a running JiGong instance:

- The operator installs the plugin from a local path: `pnpm Jigongai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-Jigong-plugin`) that scaffolds:

- `package.json` with correct `JigongPlugin` keys
...
- `@Jigong/plugin-workspace-files`
- `@Jigong/plugin-terminal`
- `@Jigong/plugin-git`
- `@Jigong/plugin-linear`
- `@Jigong/plugin-github-issues`
- `@Jigong/plugin-grafana`
- `@Jigong/plugin-runtime-processes`
- `@Jigong/plugin-stripe`
...
- `@Jigongai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@Jigongai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@Jigongai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@Jigongai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@Jigongai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@Jigongai/plugin-sdk/ui`
...
- `@Jigongai/plugin-test-harness`
- `create-Jigong-plugin` starter template
...
JiGong should not implement a generic in-process hook bag modeled directly after local coding tools.

JiGong should implement:
...
That is the complete target design for the JiGong plugin system.
