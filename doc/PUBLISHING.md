...
This document covers how to build and publish the `Jigongai` CLI package to npm.

## Prerequisites

- Node.js 20+
- pnpm 9.15+
- An npm account with publish access to the `Jigongai` package
...
3. **esbuild bundle** — bundles the CLI entry point (`cli/src/index.ts`) and all workspace package code (`@Jigongai/*`) into a single file at `cli/dist/index.js`. External npm dependencies (express, postgres, etc.) are kept as regular imports.
...
{
  "dependencies": {
    "@Jigongai/server": "workspace:*",
    "@Jigongai/db": "workspace:*"
  }
}
```
...
The CLI is a monorepo package that imports code from `@Jigongai/server`, `@Jigongai/db`, `@Jigongai/shared`, and several adapter packages. These workspace packages don't exist on npm.

**esbuild** bundles all workspace TypeScript code into a single `dist/index.js` file (~250kb). External npm packages (express, postgres, zod, etc.) are left as normal `import` statements — they get installed by npm when a user runs `npx Jigongai onboard`.
