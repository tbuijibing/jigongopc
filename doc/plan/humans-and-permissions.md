...
- local mode that still feels instant (`npx Jigongai run` and go)
...
2. CLI command `pnpm Jigongai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm Jigongai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm Jigongai onboard`).
...
- expose key instance settings in API + CLI (`Jigongai configure` / `Jigongai onboard`)
...
7. `pnpm Jigongai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
