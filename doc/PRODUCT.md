# JiGong — Product Definition

## What It Is

JiGong is the control plane for autonomous AI companies. One instance of JiGong can run multiple companies. A **company** is a first-order object.
...
- **Adapter type + config** — how this agent runs and what defines its identity/behavior. This is adapter-specific (e.g., an OpenClaw agent might use SOUL.md and HEARTBEAT.md files; a Claude Code agent might use CLAUDE.md; a bare script might use CLI args). JiGong doesn't prescribe the format — the adapter does.
...
1. **Run a command** — JiGong kicks off a process (shell command, Python script, etc.) and tracks it. The heartbeat is "execute this and monitor it."
2. **Fire and forget a request** — JiGong sends a webhook/API call to an externally running agent. The heartbeat is "notify this agent to wake up." (OpenClaw hooks work this way.)
...
1. **Unopinionated about how you run your agents.** Your agents could be OpenClaw bots, Python scripts, Node scripts, Claude Code sessions, Codex instances — we don't care. JiGong defines the control plane for communication and provides utility infrastructure for heartbeats. It does not mandate an agent runtime.

2. **Company is the unit of organization.** Everything lives under a company. One JiGong instance, many companies.
...
5. **Control plane, not execution plane.** JiGong orchestrates. Agents run wherever they run and phone home.

## User Flow (Dream Scenario)

1. Open JiGong, create a new company
...
There are two runtime modes JiGong must support:
