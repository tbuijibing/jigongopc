...
Audience: Operators setting up and running agents in JiGong

## 1. What this system does

Agents in JiGong do not run continuously.  
...
For `claude_local` and `codex_local`, JiGong assumes the CLI is already installed and authenticated on the host machine.
...
JiGong stores resumable session state per `(agent, taskKey, adapterType)`.
...
JiGong pushes runtime/activity updates to the browser in real time.
