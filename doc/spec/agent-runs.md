...
This spec defines how JiGong actually runs agents while staying runtime-agnostic.
...
1. JiGong is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, JiGong manages lifecycle, and on exit JiGong parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `Jigong.skill` (shared JiGong skill text block)
...
2. Preferred transport is env vars (`Jigong_*`) injected at runtime.
