...
const Jigong_SESSIONS_DIR = path.join(os.homedir(), ".pi", "Jigongs");
...
async function ensurePiSkillsInjected(onLog: AdapterExecutionContext["onLog"]) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[Jigong] Injected Pi skill "${entry.name}" into ${piSkillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[Jigong] Failed to inject Pi skill "${entry.name}" into ${piSkillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(Jigong_SESSIONS_DIR, { recursive: true });
  return Jigong_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(Jigong_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (wakeTaskId) env.Jigong_TASK_ID = wakeTaskId;
  if (wakeReason) env.Jigong_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.Jigong_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.Jigong_APPROVAL_ID = approvalId;
  if (approvalStatus) env.Jigong_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.Jigong_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (workspaceCwd) env.Jigong_WORKSPACE_CWD = workspaceCwd;
  if (workspaceSource) env.Jigong_WORKSPACE_SOURCE = workspaceSource;
  if (workspaceId) env.Jigong_WORKSPACE_ID = workspaceId;
  if (workspaceRepoUrl) env.Jigong_WORKSPACE_REPO_URL = workspaceRepoUrl;
  if (workspaceRepoRef) env.Jigong_WORKSPACE_REPO_REF = workspaceRepoRef;
  if (workspaceHints.length > 0) env.Jigong_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[Jigong] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[Jigong] Loaded agent instructions file: ${resolvedInstructionsFilePath}\n`,
      );
    } catch (err) {
      ...
      await onLog(
        "stderr",
        `[Jigong] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[Jigong] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
