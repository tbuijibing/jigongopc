...
import type { AdapterModel } from "@Jigongai/adapter-utils";
import { asString, runChildProcess } from "@Jigongai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.Jigong_PI_COMMAND === "string" &&
    process.env.Jigong_PI_COMMAND.trim().length > 0
      ? process.env.Jigong_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["Jigong_", "npm_", "NPM_"] as const;
