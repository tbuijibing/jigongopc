...
Use when:
- You want JiGong to run Pi (the AI coding agent) locally as the agent runtime
...
- JiGong requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/Jigongs/ and resumed with --session.
