...
Use when:
- You want JiGong to run Cursor Agent CLI locally as the agent runtime
...
- JiGong auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$Jigong" and related skills on local runs.
- JiGong auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
