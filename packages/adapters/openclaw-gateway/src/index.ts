...
Use when:
- You want JiGong to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the JiGong server.
...
- JigongApiUrl (string, optional): absolute JiGong base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default Jigong)
`;
