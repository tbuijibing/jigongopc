...
import { asString, parseObject } from "@Jigongai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the JiGong server host.",
        });
      }
    } catch (err) {
      checks.push({
