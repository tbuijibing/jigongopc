...
## Scope
This plan is now **gateway-only**. JiGong supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses JiGong skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a JiGong task.
...
```bash
pnpm --filter @Jigongai/server test:run -- openclaw-gateway
pnpm --filter @Jigongai/server typecheck
pnpm --filter @Jigongai/ui typecheck
pnpm --filter Jigongai typecheck
```
