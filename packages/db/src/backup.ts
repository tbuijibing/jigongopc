...
function resolveJiGongHomeDir(): string {
  const envHome = process.env.Jigong_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".jigong");
}

function resolveJiGongInstanceId(): string {
  const raw = process.env.Jigong_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveJiGongHomeDir(), "instances", resolveJiGongInstanceId(), "config.json");
}
...
function resolveConnectionString(config: PartialConfig | null): string {
  ...
  return `postgres://jigong:jigong@127.0.0.1:${port}/jigong`;
}

function resolveDefaultBackupDir(): string {
  return path.resolve(resolveJiGongHomeDir(), "instances", resolveJiGongInstanceId(), "data", "backups");
}
