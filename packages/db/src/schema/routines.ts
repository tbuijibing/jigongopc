...
export const routineRuns = pgTable(
  ...
  (table) => ({
    routineStatusIdx: index("routine_runs_routine_status_idx").on(table.routineId, table.status),
    companyCreatedAtIdx: index("routine_runs_company_created_idx").on(table.companyId, table.createdAt),
  }),
);
