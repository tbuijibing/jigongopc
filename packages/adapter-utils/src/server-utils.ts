...
export function buildJiGongEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.Jigong_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.Jigong_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.Jigong_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.Jigong_API_URL = apiUrl;
  return vars;
}
