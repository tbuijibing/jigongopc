...
name: release
description: >
  Coordinate a full JiGong release across engineering, website publishing,
...
Run the full JiGong release process as an organizational workflow, not just
an npm publish.
...
6. If running via JiGong, you have issue context for posting status updates.
...
- Release request source issue (if in JiGong)
...
| Canary publish | `npm view Jigongai@{version}` succeeds | Skip canary publish. Proceed to smoke test. |
...
| Website task | Search JiGong issues for "Publish release notes for v{version}" | Skip creation. Link the existing task. |
| CMO task | Search JiGong issues for "release announcement tweet for v{version}" | Skip creation. Link the existing task. |
...
npm view Jigongai@{version} version 2>/dev/null && echo "ALREADY_PUBLISHED" || echo "NOT_PUBLISHED"
...
is **not** updated. Users running `npx Jigongai onboard` still get the
previous stable version.

After publish, verify the canary is accessible:

```bash
npm view Jigongai@canary version
...
Run the canary in a clean Docker environment to verify `npx Jigongai onboard`
works end-to-end.
...
```bash
JigongAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

This builds a clean Ubuntu container, installs `Jigongai@canary` via npx, and
...
`npx Jigongai onboard` picks up the new version.

### Promote on npm

```bash
# For each published package, move the dist-tag from canary to latest
npm dist-tag add Jigongai@{version} latest
npm dist-tag add @Jigongai/server@{version} latest
npm dist-tag add @Jigongai/cli@{version} latest
npm dist-tag add @Jigongai/shared@{version} latest
npm dist-tag add @Jigongai/db@{version} latest
npm dist-tag add @Jigongai/adapter-utils@{version} latest
npm dist-tag add @Jigongai/adapter-claude-local@{version} latest
npm dist-tag add @Jigongai/adapter-codex-local@{version} latest
npm dist-tag add @Jigongai/adapter-openclaw-gateway@{version} latest
```
...
```bash
npm view Jigongai@latest version
# Should now show the new version

# Final sanity check
npx --yes Jigongai@latest --version
```
...
Create at least two tasks in JiGong (only if they don't already exist):
...
## JiGong API Notes (When Running in Agent Context)
...
- `Authorization: Bearer $Jigong_API_KEY`
- `X-JiGong-Run-Id: $Jigong_RUN_ID`
