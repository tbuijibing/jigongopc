...
description: >
  Technical guide for creating a new JiGong agent adapter. Use when building
...
# Creating a JiGong Agent Adapter

An adapter bridges JiGong's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `Jigongai run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@Jigongai/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@Jigongai/adapter-utils` (types) or `@Jigongai/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell JiGong to forget the stored session
}
...
```json
{
  ...
  "dependencies": {
    "@Jigongai/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including JiGong agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@Jigongai/adapter-utils/server-utils`)
2. **Build environment** — call `buildJiGongEnv(agent)` then layer in `Jigong_RUN_ID`, context vars (`Jigong_TASK_ID`, `Jigong_WAKE_REASON`, `Jigong_WAKE_COMMENT_ID`, `Jigong_APPROVAL_ID`, `Jigong_APPROVAL_STATUS`, `Jigong_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| Variable | Source |
|----------|--------|
| `Jigong_AGENT_ID` | `agent.id` |
| `Jigong_COMPANY_ID` | `agent.companyId` |
| `Jigong_API_URL` | Server's own URL |
| `Jigong_RUN_ID` | Current run id |
| `Jigong_TASK_ID` | `context.taskId` or `context.issueId` |
| `Jigong_WAKE_REASON` | `context.wakeReason` |
| `Jigong_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `Jigong_APPROVAL_ID` | `context.approvalId` |
| `Jigong_APPROVAL_STATUS` | `context.approvalStatus` |
| `Jigong_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `Jigong_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `Jigongai run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@Jigongai/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@Jigongai/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use JiGong API
...
import { parseMyAgentStdoutLine } from "@Jigongai/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@Jigongai/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@Jigongai/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so JiGong wipes the stale session
...
Import from `@Jigongai/adapter-utils/server-utils`:
...
| `buildJiGongEnv(agent)` | Standard `Jigong_*` env vars |
...
- Package name: `@Jigongai/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your JiGong work."`
...
### JiGong Skills Injection

JiGong ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `Jigong` API skill and the `Jigong-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with JiGong internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("Jigong-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Jigong-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(Jigong_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(Jigong_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks JiGong skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The JiGong skills (like `Jigong` and `Jigong-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the JiGong API to report status), use explicit instructions in the prompt template: "Use the Jigong skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between JiGong's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `Jigong_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the JiGong API and GitHub should not have access to arbitrary hosts.
