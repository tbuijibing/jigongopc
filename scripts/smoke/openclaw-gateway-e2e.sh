...
Jigong_API_URL="${Jigong_API_URL:-http://127.0.0.1:3100}"
API_BASE="${Jigong_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-Jigong-smoke}"
...
Jigong_API_URL_FOR_OPENCLAW="${Jigong_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
OPENCLAW_ADAPTER_TIMEOUT_SEC="${OPENCLAW_ADAPTER_TIMEOUT_SEC:-600}"
OPENCLAW_ADAPTER_WAIT_TIMEOUT_MS="${OPENCLAW_ADAPTER_WAIT_TIMEOUT_MS:-600000}"
PAIRING_AUTO_APPROVE="${PAIRING_AUTO_APPROVE:-1}"
PAYLOAD_TEMPLATE_MESSAGE_APPEND="${PAYLOAD_TEMPLATE_MESSAGE_APPEND:-}"

AUTH_HEADERS=()
if [[ -n "${Jigong_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${Jigong_AUTH_HEADER}" )
fi
if [[ -n "${Jigong_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${Jigong_COOKIE}" )
  Jigong_BROWSER_ORIGIN="${Jigong_BROWSER_ORIGIN:-${Jigong_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${Jigong_BROWSER_ORIGIN}" -H "Referer: ${Jigong_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set Jigong_COOKIE or Jigong_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg JigongApiUrl "$Jigong_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/Jigong"
  local claimed_file="${workspace_dir}/Jigong-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/Jigong-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/Jigong-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/Jigong-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/Jigong"
    assert_status "200"
    {
      echo "Jigong_API_URL=${Jigong_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set Jigong_API_KEY=${AGENT_API_KEY} in your run context before running Jigong heartbeat steps."
...
main() {
  ...
  wait_http_ready "${Jigong_API_URL%/}/api/health" 15 || fail "Jigong API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "Jigong health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
