...
Jigong_HOST_PORT="${Jigong_HOST_PORT:-3100}"
Jigong_HOST_FROM_CONTAINER="${Jigong_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.Jigong-openclaw.override.yml"
...
detect_Jigong_base_url() {
  ...
  for candidate in "$Jigong_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${Jigong_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${Jigong_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
Jigong_base_url="$(detect_Jigong_base_url || true)"
...
Jigong URL for OpenClaw container:
EOF
  if [[ -n "$Jigong_base_url" ]]; then
    cat <<EOF
  $Jigong_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${Jigong_HOST_PORT}
  (Do not use http://127.0.0.1:${Jigong_HOST_PORT} inside the container.)
  If Jigong rejects the host, run on host machine:
    pnpm Jigongai allowed-hostname host.docker.internal
  Then restart Jigong and re-run this script.
EOF
  fi
  cat <<EOF
