...
COMPANY_ID="${COMPANY_ID:-${Jigong_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-Jigong-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-Jigong-openclaw-smoke}"
...
Provide one of:
  Jigong_AUTH_HEADER=\"Bearer <board-token>\"
  Jigong_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Jigong OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.Jigong.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
