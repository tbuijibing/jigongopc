...
")
  ...
  if [ "$dry_run" = true ]; then
    ...
    echo ""
    echo "Verify: npm view Jigongai@latest version"
    echo ""
    echo "To push:"
...
pnpm --filter @Jigongai/shared build
pnpm --filter @Jigongai/adapter-utils build
pnpm --filter @Jigongai/db build
pnpm --filter @Jigongai/adapter-claude-local build
pnpm --filter @Jigongai/adapter-codex-local build
pnpm --filter @Jigongai/adapter-opencode-local build
pnpm --filter @Jigongai/adapter-openclaw-gateway build
pnpm --filter @Jigongai/server build

# Build UI and bundle into server package for static serving
pnpm --filter @Jigongai/ui build
rm -rf "$REPO_ROOT/server/ui-dist"
...
if [ "$canary" = true ]; then
  if [ "$dry_run" = true ]; then
    ...
    echo ""
    echo "Verify: npm view Jigongai@canary version"
    echo ""
  ...
  echo "  git push && git push origin v$NEW_VERSION"
  echo ""
  echo "GitHub Release: https://github.com/cryppadotta/Jigong/releases/tag/v$NEW_VERSION"
fi
